...
function initPlayer(newAudioInfo, isAutoPlay) {
    ...
    function play(audioid) {
        ...
        if (audio.paused) {
            ...
            $("#audioPlay1").attr("src", "http://audio.leting.io/" + $("#music" + audioid).attr("audio"));
...
function songdom(results) {
    ...
    for (var index = 0; index < results.length; index++) {
        ...
        model = model.replace(/{{newstime}}/g, getDateDiff(element['collect_time']))
...
function fetchNewsData() {
    data = {}
    data.maxid = minid
    data.catalogid = getUrlParam("catalogid")
    fetchdataing = true
    $.ajax({
        type: "GET",
        url: "/api/news_v1",
